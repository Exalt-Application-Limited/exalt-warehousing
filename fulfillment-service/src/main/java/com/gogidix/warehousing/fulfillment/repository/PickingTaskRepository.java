...
public interface PickingTaskRepository extends JpaRepository<PickingTask, UUID> {
     ...
     */
    @Query("SELECT t FROM PickingTask t WHERE t.status = com.gogidix.warehousing.fulfillment.enums.TaskStatus.PENDING " +
     ...
     */
    @Query("SELECT t FROM PickingTask t WHERE t.status = com.gogidix.warehousing.fulfillment.enums.TaskStatus.IN_PROGRESS " +
