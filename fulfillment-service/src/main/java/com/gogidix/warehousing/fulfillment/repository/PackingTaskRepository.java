...
public interface PackingTaskRepository extends JpaRepository<PackingTask, UUID> {
    ...
    @Query("SELECT t FROM PackingTask t WHERE t.dueBy < :currentTime " +
           "AND t.status NOT IN (com.gogidix.warehousing.fulfillment.enums.TaskStatus.COMPLETED, " +
           "com.gogidix.warehousing.fulfillment.enums.TaskStatus.CANCELLED)")
     ...
     */
    @Query("SELECT t FROM PackingTask t WHERE t.status = com.gogidix.warehousing.fulfillment.enums.TaskStatus.PENDING " +
     ...
     */
    @Query("SELECT t FROM PackingTask t WHERE t.status = com.gogidix.warehousing.fulfillment.enums.TaskStatus.IN_PROGRESS " +
    ...
    @Query("SELECT AVG(FUNCTION('TIMESTAMPDIFF', MINUTE, t.startedAt, t.completedAt)) FROM PackingTask t " +
           "WHERE t.status = com.gogidix.warehousing.fulfillment.enums.TaskStatus.COMPLETED " +
